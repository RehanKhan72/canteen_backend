...
export default class KitchenScheduler {
    static async ensureIndexes() {
        const db = getDb();
        await db.collection("OrderHistory").createIndex({ status: 1, kitchenScheduledAt: 1 });
        ...
        setInterval(async () => {
            ...
            for (const order of updatedOrders) {
                ...
                try {
                    ...
                    if (token) {
                        await FCMService.sendNotificationToTokens([token], "Order Update", "Your order is now being prepared", {
                            orderId: order._id,
                            status: 2
                        });
                    }
